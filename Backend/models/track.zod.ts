...
interface IQuizQuestion {
  question: string;
  options: {
    option_heading: string;
    feedback_text: string;
    isCorrect: boolean;
    translations: object;
  }[];
  type: string; // radio | checkbox | textinput
  explanation: string;
  translations: object;
}
...
interface IResource {
  ...
  quiz?: {
    ...
    // marks: number;
    estimatedDuration?: number;
    gradedPassedText: string;
    gradedFailedText: string;
    gradedPassingCriteria: number;
    gradedReattemptCount: number;
    non_gradedResultText: string;
  };
  activity?: {
    ...
    isFinalPresentationActivity: boolean;
    passing_criteria: number;
    evaluationInstToAI: string;
    submissionType: "image" | "doc" | "video" | "text";
    subbmissionSize: number;
    isTextFieldRequired: boolean;
    textFieldCharCount: number;
  };
  ppt?: {
    fileUrl?: string;
    downloadable?: boolean;
...
export interface ITrack extends Document {
  ...
  goodFor?: string;
  duration: 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8; // '1 year' | '2 years' | '3 years' | '4 years';
  status: "DRAFT" | "LIVE";
  ...
  trackIcon: string;
  terms: ITerm[];
  learn_more_resources: IResource[];
...
const ResourceSchema = new Schema<IResource>(
  {
    ...
    activity: {
      ...
      // isRevenueRequired: {type: Boolean},
      // isUploadRequired: {type: Boolean},
      isEvaluationRequired: { type: Boolean },
